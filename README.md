...
> Este README segue a estrutura de tópicos exigida na especificação do MVP. Trechos
> marcados como `[INSERIR PRINT]` ou `PREENCHER APÓS EXECUÇÃO` devem ser completados
> por você depois de rodar os notebooks no seu próprio workspace Databricks, com
> evidências (screenshots) reais do seu ambiente — isso é exigido pela especificação
> do trabalho e é o que comprova que o pipeline foi de fato executado.
...
`[INSERIR PRINT]` — resultado do `display()` do notebook 01 e a tabela
`workspace.bronze.triagem_asd_raw` visível no Catalog Explorer.
...
`[INSERIR PRINT]` — telas do Catalog Explorer mostrando o schema `gold` com as 5
tabelas e, se disponível, a aba "Lineage" evidenciando a linhagem Bronze → Silver → Gold.
...
`[INSERIR PRINT]` — evidências de que as tabelas Bronze, Silver e Gold foram
persistidas (screenshots do Catalog Explorer / resultado de `SHOW TABLES`).
...
![Acurácia - pontuação](evidencias/04_qualidade_dados/07_acuracia_pontuacao.png)
...
![Outliers](evidencias/04_qualidade_dados/08_outliers_boxplot.png)
...
![Resultado por faixa etária e gênero](evidencias/05_analise/01_faixa_etaria_genero.png)
...
![Histórico familiar de autismo](evidencias/05_analise/02_historico_familiar.png)
...
![Icterícia ao nascer](evidencias/05_analise/03_ictericia.png)
...
![Quem respondeu o teste](evidencias/05_analise/04_quem_respondeu.png)
...
![País de residência](evidencias/05_analise/05_pais_residencia.png)
...
![Uso prévio de aplicativo de triagem](evidencias/05_analise/06_uso_app_antes.png)
...
![Percentual de triagens positivas por faixa etária](evidencias/05_analise/07_grafico_faixa_etaria.png)
...
PREENCHER APÓS EXECUÇÃO. Sugestão de roteiro para esta seção:

- **Objetivos atingidos:** quais das 6 perguntas você conseguiu responder de forma
  satisfatória, com que grau de confiança nos resultados.
- **Objetivos não atingidos (e por quê):** por exemplo, se alguma pergunta ficou
  limitada pelo tamanho da amostra (704 registros é uma base pequena para alguns
  recortes, como por país), ou por limitações do próprio dataset (não há dados
  longitudinais/temporais, não há diagnóstico clínico confirmado, apenas o
  resultado da triagem).
- **Dificuldades encontradas:** técnicas (ex.: configuração do Databricks Free
  Edition, Unity Catalog, Volumes) e conceituais (ex.: modelagem dimensional para
  um dataset sem componente temporal).
- **Trabalhos futuros:** por exemplo, enriquecer o pipeline com dados adicionais
  (ex.: bases de diagnóstico clínico confirmado para validar a triagem), adicionar
  uma dimensão de tempo caso se obtenha um dataset com data de aplicação do teste,
  ou aplicar um modelo preditivo (ex.: classificação) sobre a camada Gold para
  estimar a probabilidade de resultado positivo a partir do perfil do respondente.
